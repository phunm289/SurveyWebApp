...
public class Control {
    ...
    public String processingJSON(String strJSON) {
            ...
            switch (type) {
                case "text":
                    ...
                    if ("true".equals(options.getString("valid"))) {
                        ...
                        htmlContent += "<textarea name=\"textanswer-" + (i + 1) + "\" style=\"box-sizing: border-box;min-width:60%;width:100%;max-width:100%;max-height:100px;\" class=\"form-control\" rows=\"3\" id=\"text_answer_" + (i + 1) + "\" onblur=\"validate('text_answer_" + (i + 1) + "','" + valid_type + "','" + valid_condition + "','" + value_1 + "','" + value_2 + "');\">\n</textarea>\n";
                    } else {
                        htmlContent += "<textarea name=\"textanswer-" + (i + 1) + "\" style=\"box-sizing: border-box;min-width:60%;width:100%;max-width:100%;max-height:100px;\" id=\"text_answer_" + (i + 1) + "\" class=\"form-control\" rows=\"3\">\n</textarea>\n";
                    }
                    htmlContent += "</div>\n";
                    break;
                case "radio button":
                    ...
                    for (int x = 0; x < options.length(); x++) {
                        ...
                        htmlContent += "<label>\n<input style=\"margin:0px 5px;\" type=\"radio\" name=\"radio_" + (i + 1) + "\" id=\"optionRadio_" + (i + 1) + "_" + (x + 1) + "" + "\" value=\"rdo_option_" + (x + 1) + "\">\n" + option + "</label>\n";
                    }
                    htmlContent += "</div>\n";
                    break;
                case "check box":
                    ...
                    for (int x = 0; x < options.length(); x++) {
                        String option = options.getString("" + x);
                        htmlContent += "<label>\n<input style=\"margin:0px 5px;\" type=\"checkbox\" name=\"checkbox_" + (i + 1) + "\" id=\"optionCheckbox_" + (i + 1) + "_" + (x + 1) + "" + "\" value=\"cb_option_" + (x + 1) + "\">\n" + option + "</label>\n";
                    }
                    htmlContent += "</div>\n";
                    break;
                case "combobox":
                    ...
                    htmlContent += "<select id=\"dropdown_" + (i + 1) + "\" class=\"form-control\">\n";
                    for (int x = 0; x < options.length(); x++) {
                        String option = options.getString("" + x);
                ...
                case "rating":
                    ...
                    for (int j = 0; j < totalOption; j++) {
                        htmlContent += "<input style=\"margin:0px 5px;\" type=\"radio\" name=\"optionsRating\" id=\"optionsRating_" + (i + 1) + "_" + (j + 1) + "\" value=\"optionsRating_" + (i + 1) + "_" + (j + 1) + "\">\n";
                    }
                ...
                case "table":
                    ...
                    for (int y = 0; y < jsonRow.length(); y++) {
                        ...
                        for (int x = 0; x < jsonCol.length(); x++) {
                            ...
                            htmlContent += "<input style=\"margin:0px 5px;\" type=\"radio\" name=\"tbl_content_row_" + (y + 1) + "\" id=\"row" + (y + 1) + "_col" + (x + 1) + "\" value=\"" + row + "_" + col + "\">\n";
                            htmlContent += "</div>\n";
                            htmlContent += "</td>\n";
                        }
                        htmlContent += "</tr>\n";
                ...
                case "date":
                    ...
                    htmlContent += "<div id=\"date_selector\">\n<input type=\"text\" id=\"datepicker_" + (i + 1) + "\" readonly style=\"cursor:text;\" onclick=\"showdatepicker('datepicker_" + (i + 1) + "');\"/>\n</div>\n";
                    htmlContent += "</div>\n";
                    break;
            }
            htmlContent += "</div>\n";
